...
def get_short_name(field_name):
    return ''.join([a[0] for a in field_name.split('_')])
...
def get_filename(hparams, setting_dict, priority):
    ...
    for field, val in setting_dict.items():
        ...
        filename_list.append(str(val))
    filename = '_'.join(filename_list).encode('utf-8')
...
def create_scripts(hparams, base_script, grid):
    ...
    for setting in itertools.product(*grid.values()):
        ...
        for (idx, value) in enumerate(setting):
            setting_dict[list(grid.keys())[idx]] = value
...
if __name__ == '__main__':
    ...
    PARSER.add_argument('--base-script-path', type=str, default='./mnist/base_script.sh', help='Path to base script')
    PARSER.add_argument('--grid-path', type=str, default='./mnist/grid_wgangp_drop_independent.sh', help='Path to a file that specifies the grid')
